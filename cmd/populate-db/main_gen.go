...
func main() {
	...
	flagInstancesMin := pflag.Int("instances-min", 2, "min instances to create per cluster")
	flagInstancesMax := pflag.Int("instances-max", 5, "max instances to create per cluster")
	flagNetworksMin := pflag.Int("networks-min", 2, "min networks to create per cluster")
	flagNetworksMax := pflag.Int("networks-max", 5, "max networks to create per cluster")
	flagNetworkACLsMin := pflag.Int("network-acls-min", 2, "min network ACLs to create per cluster")
	flagNetworkACLsMax := pflag.Int("network-acls-max", 5, "max network ACLs to create per cluster")
	flagNetworkAddressSetsMin := pflag.Int("network-address-sets-min", 2, "min network address sets to create per cluster")
	flagNetworkAddressSetsMax := pflag.Int("network-address-sets-max", 5, "max network address sets to create per cluster")
	flagNetworkForwardsMin := pflag.Int("network-forwards-min", 2, "min network forwards to create per cluster")
	flagNetworkForwardsMax := pflag.Int("network-forwards-max", 5, "max network forwards to create per cluster")
	flagNetworkIntegrationsMin := pflag.Int("network-integrations-min", 2, "min network integrations to create per cluster")
	flagNetworkIntegrationsMax := pflag.Int("network-integrations-max", 5, "max network integrations to create per cluster")
	flagNetworkLoadBalancersMin := pflag.Int("network-load-balancers-min", 2, "min network load balancers to create per cluster")
	flagNetworkLoadBalancersMax := pflag.Int("network-load-balancers-max", 5, "max network load balancers to create per cluster")
	flagNetworkPeersMin := pflag.Int("network-peers-min", 2, "min network peers to create per cluster")
	flagNetworkPeersMax := pflag.Int("network-peers-max", 5, "max network peers to create per cluster")
	flagNetworkZonesMin := pflag.Int("network-zones-min", 2, "min network zones to create per cluster")
	flagNetworkZonesMax := pflag.Int("network-zones-max", 5, "max network zones to create per cluster")
	flagProfilesMin := pflag.Int("profiles-min", 2, "min profiles to create per cluster")
	...
	flagStorageBucketsMin := pflag.Int("storage-buckets-min", 2, "min storage buckets to create per cluster")
	flagStorageBucketsMax := pflag.Int("storage-buckets-max", 5, "max storage buckets to create per cluster")
	flagStoragePoolsMin := pflag.Int("storage-pools-min", 2, "min storage pools to create per cluster")
	flagStoragePoolsMax := pflag.Int("storage-pools-max", 5, "max storage pools to create per cluster")
	flagStorageVolumesMin := pflag.Int("storage-volumes-min", 2, "min storage volumes to create per cluster")
