...
import (
	...
	"github.com/FuturFusion/operations-center/cmd/generate-database/file"
	"github.com/FuturFusion/operations-center/cmd/generate-database/lex"
)
...
type Method struct {
	...
	mapErr bool              // If set to true, logic to call mapErr is included in order to customize the returned errors
}

// NewMethod return a new method code snippet for executing a certain mapping.
func NewMethod(database, pkg, entity, kind string, config map[string]string, mapErr bool) (*Method, error) {
	var pkgPath string
	if pkg != "" {
		...
		for i, filter := range filters {
			...
			for _, name := range filter {
				for _, field := range mapping.Fields {
					if name == field.Name && isTrue(field.Config.Get("marshal")) {
