...
type parser struct {
	...
	exprStructs map[string]*ast.GenDecl
...
func (p *parser) CopyStruct(structName string, targetFile string) error {
	p.exprStructs = map[string]*ast.GenDecl{}
	...
	err = os.WriteFile(targetFile, []byte("// Code generated by generate-expr. DO NOT EDIT.\n\n"), 0o644)
	if err != nil {
		return err
	}
...
func (p *parser) generateStruct(strct *Struct) bool {
	...
	if overwroteAnyTag {
		...
		p.exprStructs[exprName] = structDecl
		p.converters = append(p.converters, p.generateConverter(exprName, strct))
	}
