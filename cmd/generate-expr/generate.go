...
func generate(pkgPaths []string) error {
	...
	glob, err := filepath.Glob("*_expr_gen.go")
	if err != nil {
	...
	f, err := os.Create("utils_expr_gen.go")
	if err != nil {
		return err
	}
		...
		for _, goFile := range parsedPkg.CompiledGoFiles {
			...
			for line := range strings.SplitSeq(string(body), "\n") {
				...
				if ok {
					...
					err = p.CopyStruct(args[0], lex.SnakeCase(args[0])+"_expr_gen.go")
					if err != nil {
						return err
					}
				}
