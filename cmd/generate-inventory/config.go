...
type Entity struct {
	...
	DisplayAttributes      []DisplayAttribute `yaml:"display-attributes"`        // List of display attributes to be shown in the cli (show command and optionally also in list command).
	ColumnPipelines        []ColumnPipeline   `yaml:"column-pipelines"`          // List of Go template pipelines, that are applied to the value of the respective column. Go template functions from https://github.com/Masterminds/sprig are available.
...
type DisplayAttribute struct {
	...
	ShowInList bool   `yaml:"show-in-list"`
}

type ColumnPipeline struct {
	Name          string `yaml:"name"`
	Pipeline      string `yaml:"pipeline"`
	EnabledInList bool   `yaml:"enabled-in-list"`
	EnabledInShow bool   `yaml:"enabled-in-show"`
}
