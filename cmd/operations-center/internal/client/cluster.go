...
func (c OperationsCenterClient) GetClusters() ([]api.Cluster, error) {
	response, err := c.doRequest(http.MethodGet, "/provisioning/clusters", "recursion=1", nil)
	if err != nil {
		return nil, err
	}
...
func (c OperationsCenterClient) GetCluster(name string) (api.Cluster, error) {
	response, err := c.doRequest(http.MethodGet, path.Join("/provisioning/clusters", name), "", nil)
	if err != nil {
...
func (c OperationsCenterClient) CreateCluster(cluster api.ClusterPost) error {
	...
	response, err := c.doRequest(http.MethodPost, "/provisioning/clusters", "", content)
	if err != nil {
		return err
	}
...
func (c OperationsCenterClient) DeleteCluster(name string) error {
	_, err := c.doRequest(http.MethodDelete, path.Join("/provisioning/clusters", name), "", nil)
	if err != nil {
		return err
	}
...
func (c OperationsCenterClient) ResyncCluster(name string) error {
	_, err := c.doRequest(http.MethodPost, path.Join("/provisioning/clusters", name, "resync-inventory"), "", nil)
	if err != nil {
		return err
	}
