...
const networkDefaultColumns = `{{ .UUID }},{{ .Name }},{{ .ProjectName }},{{ .Cluster }},{{ .LastUpdated }}`
...
func (c *cmdNetworkList) Run(cmd *cobra.Command, args []string) error {
	...
	for i, field := range fields {
		...
		sorter, ok := networkColumnSorters[title]
		if ok {
			sorter.Index = i
	...
	for _, network := range networks {
		network.LastUpdated = network.LastUpdated.Truncate(time.Second)
...
func (c *cmdNetworkShow) Run(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("Last Updated: %s\n", network.LastUpdated.Truncate(time.Second).String())
