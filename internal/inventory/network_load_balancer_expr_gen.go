// Code generated by generate-expr. DO NOT EDIT.

package inventory

import (
	"time"

	"github.com/google/uuid"
	"github.com/lxc/incus/v6/shared/api"
)

type ExprApiNetworkLoadBalancer struct {
	ExprApiNetworkLoadBalancerPut `yaml:",inline"`
	ListenAddress                 string `json:"listen_address" yaml:"listen_address" expr:"listen_address"`
	Location                      string `json:"location" yaml:"location" expr:"location"`
}
type ExprApiNetworkLoadBalancerBackend struct {
	Name          string `json:"name" yaml:"name" expr:"name"`
	Description   string `json:"description" yaml:"description" expr:"description"`
	TargetPort    string `json:"target_port" yaml:"target_port" expr:"target_port"`
	TargetAddress string `json:"target_address" yaml:"target_address" expr:"target_address"`
}
type ExprApiNetworkLoadBalancerPort struct {
	Description   string   `json:"description" yaml:"description" expr:"description"`
	Protocol      string   `json:"protocol" yaml:"protocol" expr:"protocol"`
	ListenPort    string   `json:"listen_port" yaml:"listen_port" expr:"listen_port"`
	TargetBackend []string `json:"target_backend" yaml:"target_backend" expr:"target_backend"`
}
type ExprApiNetworkLoadBalancerPut struct {
	Description string                              `json:"description" yaml:"description" expr:"description"`
	Config      api.ConfigMap                       `json:"config" yaml:"config" expr:"config"`
	Backends    []ExprApiNetworkLoadBalancerBackend `json:"backends" yaml:"backends" expr:"backends"`
	Ports       []ExprApiNetworkLoadBalancerPort    `json:"ports" yaml:"ports" expr:"ports"`
}
type ExprNetworkLoadBalancer struct {
	ID          int                        `json:"id" expr:"id"`
	UUID        uuid.UUID                  `json:"uuid" expr:"uuid"`
	Cluster     string                     `json:"cluster" expr:"cluster"`
	NetworkName string                     `json:"network_name" expr:"network_name"`
	Name        string                     `json:"name" expr:"name"`
	Object      ExprApiNetworkLoadBalancer `json:"object" expr:"object"`
	LastUpdated time.Time                  `json:"last_updated" expr:"last_updated"`
}

func ToExprApiNetworkLoadBalancer(n api.NetworkLoadBalancer) ExprApiNetworkLoadBalancer {
	return ExprApiNetworkLoadBalancer{ExprApiNetworkLoadBalancerPut: ToExprApiNetworkLoadBalancerPut(n.NetworkLoadBalancerPut), ListenAddress: n.ListenAddress, Location: n.Location}
}
func ToExprApiNetworkLoadBalancerBackend(n api.NetworkLoadBalancerBackend) ExprApiNetworkLoadBalancerBackend {
	return ExprApiNetworkLoadBalancerBackend{Name: n.Name, Description: n.Description, TargetPort: n.TargetPort, TargetAddress: n.TargetAddress}
}
func ToExprApiNetworkLoadBalancerPort(n api.NetworkLoadBalancerPort) ExprApiNetworkLoadBalancerPort {
	return ExprApiNetworkLoadBalancerPort{Description: n.Description, Protocol: n.Protocol, ListenPort: n.ListenPort, TargetBackend: n.TargetBackend}
}
func ToExprApiNetworkLoadBalancerPut(n api.NetworkLoadBalancerPut) ExprApiNetworkLoadBalancerPut {
	return ExprApiNetworkLoadBalancerPut{Description: n.Description, Config: n.Config, Backends: sliceConvert(n.Backends, ToExprApiNetworkLoadBalancerBackend), Ports: sliceConvert(n.Ports, ToExprApiNetworkLoadBalancerPort)}
}
func ToExprNetworkLoadBalancer(n NetworkLoadBalancer) ExprNetworkLoadBalancer {
	return ExprNetworkLoadBalancer{ID: n.ID, UUID: n.UUID, Cluster: n.Cluster, NetworkName: n.NetworkName, Name: n.Name, Object: ToExprApiNetworkLoadBalancer(n.Object), LastUpdated: n.LastUpdated}
}
