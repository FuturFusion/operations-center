...
func (r networkACL) Create(ctx context.Context, in inventory.NetworkACL) (inventory.NetworkACL, error) {
	const sqlStmt = `
INSERT INTO network_acls (cluster_name, project_name, name, object, last_updated)
...
FROM network_acls
  INNER JOIN clusters ON network_acls.cluster_name = clusters.name
WHERE true
%s
...
func (r networkACL) GetByID(ctx context.Context, id int) (inventory.NetworkACL, error) {
	const sqlStmt = `
SELECT
  network_acls.id, network_acls.cluster_name, network_acls.project_name, network_acls.name, network_acls.object, network_acls.last_updated
FROM
  network_acls
...
func (r networkACL) DeleteByCluster(ctx context.Context, cluster string) error {
	const sqlStmt = `DELETE FROM network_acls WHERE cluster_name=:cluster;`
...
func (r networkACL) UpdateByID(ctx context.Context, in inventory.NetworkACL) (inventory.NetworkACL, error) {
	const sqlStmt = `
UPDATE network_acls SET cluster_name=:cluster_name, project_name=:project_name, name=:name, object=:object, last_updated=:last_updated
WHERE id=:id
RETURNING id, cluster_name, project_name, name, object, last_updated;
`
