...
func (r networkForward) Create(ctx context.Context, in inventory.NetworkForward) (inventory.NetworkForward, error) {
	const sqlStmt = `
INSERT INTO network_forwards (cluster_name, network_name, name, object, last_updated)
...
FROM network_forwards
  INNER JOIN clusters ON network_forwards.cluster_name = clusters.name
WHERE true
%s
...
func (r networkForward) GetByID(ctx context.Context, id int) (inventory.NetworkForward, error) {
	const sqlStmt = `
SELECT
  network_forwards.id, network_forwards.cluster_name, network_forwards.network_name, network_forwards.name, network_forwards.object, network_forwards.last_updated
FROM
  network_forwards
...
func (r networkForward) DeleteByCluster(ctx context.Context, cluster string) error {
	const sqlStmt = `DELETE FROM network_forwards WHERE cluster_name=:cluster;`
...
func (r networkForward) UpdateByID(ctx context.Context, in inventory.NetworkForward) (inventory.NetworkForward, error) {
	const sqlStmt = `
UPDATE network_forwards SET cluster_name=:cluster_name, network_name=:network_name, name=:name, object=:object, last_updated=:last_updated
WHERE id=:id
RETURNING id, cluster_name, network_name, name, object, last_updated;
`
