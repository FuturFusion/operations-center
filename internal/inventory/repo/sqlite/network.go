...
func (r network) Create(ctx context.Context, in inventory.Network) (inventory.Network, error) {
	const sqlStmt = `
INSERT INTO networks (cluster_name, project_name, name, object, last_updated)
...
SELECT networks.id
FROM networks
  INNER JOIN clusters ON networks.cluster_name = clusters.name
WHERE true
%s
...
func (r network) GetByID(ctx context.Context, id int) (inventory.Network, error) {
	const sqlStmt = `
SELECT
  networks.id, networks.cluster_name, networks.project_name, networks.name, networks.object, networks.last_updated
FROM
  networks
...
func (r network) DeleteByCluster(ctx context.Context, cluster string) error {
	const sqlStmt = `DELETE FROM networks WHERE cluster_name=:cluster;`
...
func (r network) UpdateByID(ctx context.Context, in inventory.Network) (inventory.Network, error) {
	const sqlStmt = `
UPDATE networks SET cluster_name=:cluster_name, project_name=:project_name, name=:name, object=:object, last_updated=:last_updated
WHERE id=:id
RETURNING id, cluster_name, project_name, name, object, last_updated;
`
