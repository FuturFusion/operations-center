...
// DeleteByCluster implements inventory.NetworkZoneRepo.
func (_d NetworkZoneRepoWithPrometheus) DeleteByCluster(ctx context.Context, cluster string) (err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
		...
		networkZoneRepoDurationSummaryVec.WithLabelValues(_d.instanceName, "DeleteByCluster", result).Observe(time.Since(_since).Seconds())
	}()
