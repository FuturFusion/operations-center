...
// GetAllIDs implements _sourceInventory.NetworkIntegrationRepo
func (_d NetworkIntegrationRepoWithSlog) GetAllIDs(ctx context.Context) (ia1 []int, err error) {
	_d._log.With(
		slog.Any("ctx", ctx),
	).Debug("NetworkIntegrationRepoWithSlog: calling GetAllIDs")
	defer func() {
		log := _d._log.With(
			slog.Any("ia1", ia1),
			slog.Any("err", err),
		...
		if err != nil {
			log.Error("NetworkIntegrationRepoWithSlog: method GetAllIDs returned an error")
		} else {
			log.Debug("NetworkIntegrationRepoWithSlog: method GetAllIDs finished")
		}
	}()
