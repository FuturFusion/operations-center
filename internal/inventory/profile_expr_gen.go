...
func ToExprApiProfile(p api.Profile) ExprApiProfile {
	return ExprApiProfile{ExprApiProfilePut: ToExprApiProfilePut(p.ProfilePut), Name: p.Name, UsedBy: p.UsedBy, Project: p.Project}
}
func ToExprApiProfilePut(p api.ProfilePut) ExprApiProfilePut {
	return ExprApiProfilePut{Config: p.Config, Description: p.Description, Devices: p.Devices}
}
func ToExprProfile(p Profile) ExprProfile {
	return ExprProfile{ID: p.ID, UUID: p.UUID, Cluster: p.Cluster, ProjectName: p.ProjectName, Name: p.Name, Object: ToExprApiProfile(p.Object), LastUpdated: p.LastUpdated}
}
