// Code generated by generate-expr. DO NOT EDIT.

package inventory

import (
	"time"

	"github.com/google/uuid"
	"github.com/lxc/incus/v6/shared/api"
)

type ExprApiNetworkForward struct {
	ExprApiNetworkForwardPut `yaml:",inline"`
	ListenAddress            string `json:"listen_address" yaml:"listen_address" expr:"listen_address"`
	Location                 string `json:"location" yaml:"location" expr:"location"`
}
type ExprApiNetworkForwardPort struct {
	Description   string `json:"description" yaml:"description" expr:"description"`
	Protocol      string `json:"protocol" yaml:"protocol" expr:"protocol"`
	ListenPort    string `json:"listen_port" yaml:"listen_port" expr:"listen_port"`
	TargetPort    string `json:"target_port" yaml:"target_port" expr:"target_port"`
	TargetAddress string `json:"target_address" yaml:"target_address" expr:"target_address"`
	SNAT          bool   `json:"snat" yaml:"snat" expr:"snat"`
}
type ExprApiNetworkForwardPut struct {
	Description string                      `json:"description" yaml:"description" expr:"description"`
	Config      api.ConfigMap               `json:"config" yaml:"config" expr:"config"`
	Ports       []ExprApiNetworkForwardPort `json:"ports" yaml:"ports" expr:"ports"`
}
type ExprNetworkForward struct {
	ID          int                   `json:"id" expr:"id"`
	UUID        uuid.UUID             `json:"uuid" expr:"uuid"`
	Cluster     string                `json:"cluster" expr:"cluster"`
	NetworkName string                `json:"network_name" expr:"network_name"`
	Name        string                `json:"name" expr:"name"`
	Object      ExprApiNetworkForward `json:"object" expr:"object"`
	LastUpdated time.Time             `json:"last_updated" expr:"last_updated"`
}

func ToExprApiNetworkForward(n api.NetworkForward) ExprApiNetworkForward {
	return ExprApiNetworkForward{ExprApiNetworkForwardPut: ToExprApiNetworkForwardPut(n.NetworkForwardPut), ListenAddress: n.ListenAddress, Location: n.Location}
}
func ToExprApiNetworkForwardPort(n api.NetworkForwardPort) ExprApiNetworkForwardPort {
	return ExprApiNetworkForwardPort{Description: n.Description, Protocol: n.Protocol, ListenPort: n.ListenPort, TargetPort: n.TargetPort, TargetAddress: n.TargetAddress, SNAT: n.SNAT}
}
func ToExprApiNetworkForwardPut(n api.NetworkForwardPut) ExprApiNetworkForwardPut {
	return ExprApiNetworkForwardPut{Description: n.Description, Config: n.Config, Ports: sliceConvert(n.Ports, ToExprApiNetworkForwardPort)}
}
func ToExprNetworkForward(n NetworkForward) ExprNetworkForward {
	return ExprNetworkForward{ID: n.ID, UUID: n.UUID, Cluster: n.Cluster, NetworkName: n.NetworkName, Name: n.Name, Object: ToExprApiNetworkForward(n.Object), LastUpdated: n.LastUpdated}
}
