// Code generated by generate-expr. DO NOT EDIT.

package inventory

import (
	"time"

	"github.com/google/uuid"
	"github.com/lxc/incus/v6/shared/api"
)

type ExprApiProject struct {
	ExprApiProjectPut `yaml:",inline"`
	Name              string   `json:"name" yaml:"name" expr:"name"`
	UsedBy            []string `json:"used_by" yaml:"used_by" expr:"used_by"`
}
type ExprApiProjectPut struct {
	Config      api.ConfigMap `json:"config" yaml:"config" expr:"config"`
	Description string        `json:"description" yaml:"description" expr:"description"`
}
type ExprProject struct {
	ID          int            `json:"id" expr:"id"`
	UUID        uuid.UUID      `json:"uuid" expr:"uuid"`
	Cluster     string         `json:"cluster" expr:"cluster"`
	Name        string         `json:"name" expr:"name"`
	Object      ExprApiProject `json:"object" expr:"object"`
	LastUpdated time.Time      `json:"last_updated" expr:"last_updated"`
}

func ToExprApiProject(p api.Project) ExprApiProject {
	return ExprApiProject{ExprApiProjectPut: ToExprApiProjectPut(p.ProjectPut), Name: p.Name, UsedBy: p.UsedBy}
}
func ToExprApiProjectPut(p api.ProjectPut) ExprApiProjectPut {
	return ExprApiProjectPut{Config: p.Config, Description: p.Description}
}
func ToExprProject(p Project) ExprProject {
	return ExprProject{ID: p.ID, UUID: p.UUID, Cluster: p.Cluster, Name: p.Name, Object: ToExprApiProject(p.Object), LastUpdated: p.LastUpdated}
}
