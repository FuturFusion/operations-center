...
// SyncServer implements _sourceInventory.ProfileService
func (_d ProfileServiceWithSlog) SyncServer(ctx context.Context, serverID int) (err error) {
	log := _d._log.With()
	if _d._log.Enabled(ctx, logger.LevelTrace) {
		log.With(
			slog.Any("ctx", ctx),
	...
	log.Debug("ProfileServiceWithSlog: calling SyncServer")
	defer func() {
		log := _d._log.With()
		if _d._log.Enabled(ctx, logger.LevelTrace) {
			log = _d._log.With(
		...
		if err != nil {
			log.Error("ProfileServiceWithSlog: method SyncServer returned an error")
		} else {
			log.Debug("ProfileServiceWithSlog: method SyncServer finished")
		}
	}()
