...
func (_d NetworkForwardServiceWithPrometheus) GetAllWithFilter(ctx context.Context, filter inventory.NetworkForwardFilter) (networkForwards inventory.NetworkForwards, err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
		...
		networkForwardServiceDurationSummaryVec.WithLabelValues(_d.instanceName, "GetAllWithFilter", result).Observe(time.Since(_since).Seconds())
	}()
...
func (_d NetworkForwardServiceWithPrometheus) ResyncByName(ctx context.Context, clusterName string, event domain.LifecycleEvent) (err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
		...
		networkForwardServiceDurationSummaryVec.WithLabelValues(_d.instanceName, "ResyncByName", result).Observe(time.Since(_since).Seconds())
	}()
