...
func updateFromV40(ctx context.Context, tx *sql.Tx) error {
	// v40..v41 allow NULL for servers.certificate to make the absence of a
...
INSERT INTO servers_new
  SELECT id, cluster_id, name, type, connection_url,
  CASE WHEN certificate == '' THEN NULL ELSE certificate END,
  status, hardware_data, os_data, last_updated, last_seen, public_connection_url,
  version_data, channel_id, status_detail, description, properties,
  last_status_updated, bmc_config, registration_token, system_uuid, machine_id,
  bmc_data, status_internal
  FROM servers;
DROP TABLE servers;
...
func updateFromV39(ctx context.Context, tx *sql.Tx) error {
	// v39..v40 add servers.status_internal column.
	stmt := `
	ALTER TABLE servers ADD COLUMN status_internal TEXT NOT NULL DEFAULT '{}';
`
	_, err := tx.Exec(stmt)
	return MapDBError(err)
}
...
func updateFromV38(ctx context.Context, tx *sql.Tx) error {
	// v38..v39 add servers.bmc_data column.
	stmt := `
	ALTER TABLE servers ADD COLUMN bmc_data TEXT NOT NULL DEFAULT '{}';
`
	_, err := tx.Exec(stmt)
	return MapDBError(err)
}
...
func updateFromV29(ctx context.Context, tx *sql.Tx) error {
	// v29..v30 add cluster update status
	stmt := `
ALTER TABLE clusters ADD COLUMN update_status TEXT NOT NULL DEFAULT '';
ALTER TABLE clusters ADD COLUMN config TEXT NOT NULL DEFAULT '';
`
	_, err := tx.Exec(stmt)
	return MapDBError(err)
}
