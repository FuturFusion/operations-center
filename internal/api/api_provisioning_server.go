...
func registerProvisioningServerHandler(
	...
	router.HandleFunc("POST /1.0/provisioning/servers/<servername>/bmc/:start", response.With(handler.serverBMCStartPost, assertPermission(authorizer, authz.ObjectTypeServer, authz.EntitlementCanEdit)))
	router.HandleFunc("POST /1.0/provisioning/servers/<servername>/bmc/:stop", response.With(handler.serverBMCStopPost, assertPermission(authorizer, authz.ObjectTypeServer, authz.EntitlementCanEdit)))
	router.HandleFunc("POST /1.0/provisioning/servers/<servername>/bmc/:restart", response.With(handler.serverBMCRestartPost, assertPermission(authorizer, authz.ObjectTypeServer, authz.EntitlementCanEdit)))
