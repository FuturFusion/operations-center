...
func registerProvisioningTokenHandler(router Router, authorizer authz.Authorizer, service provisioning.TokenService) {
	...
	router.HandleFunc("POST /{uuid}/iso", response.With(handler.tokenISOPost, assertPermission(authorizer, authz.ObjectTypeServer, authz.EntitlementCanView))) // TODO: is EntitlementCanView the right permission?
}
