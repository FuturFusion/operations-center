...
func (_d ServerServiceWithSlog) RestoreSystemByName(ctx context.Context, name string, force bool) (err error) {
	log := slog.With()
	if slog.Default().Enabled(ctx, logger.LevelTrace) {
		log = log.With(
			slog.Any("ctx", ctx),
	...
	return _d._base.RestoreSystemByName(ctx, name, force)
}
