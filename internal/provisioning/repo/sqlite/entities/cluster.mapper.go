...
INSERT INTO clusters (name, connection_url, certificate, status, update_status, channel_id, description, properties, last_updated)
  VALUES (?, ?, ?, ?, ?, (SELECT channels.id FROM channels WHERE channels.name = ?), ?, ?, ?)
`)

var clusterUpdate = RegisterStmt(`
UPDATE clusters
  SET name = ?, connection_url = ?, certificate = ?, status = ?, update_status = ?, channel_id = (SELECT channels.id FROM channels WHERE channels.name = ?), description = ?, properties = ?, last_updated = ?
 WHERE id = ?
`)
...
func clusterColumns() string {
	return "clusters.id, clusters.name, clusters.connection_url, clusters.certificate, clusters.status, clusters.update_status, channels.name AS channel, clusters.description, clusters.properties, clusters.last_updated"
}
...
func getClusters(ctx context.Context, stmt *sql.Stmt, args ...any) ([]provisioning.Cluster, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		...
		err := scan(&c.ID, &c.Name, &c.ConnectionURL, &c.Certificate, &c.Status, &c.UpdateStatus, &c.Channel, &c.Description, &c.Properties, &c.LastUpdated)
		if err != nil {
			return err
		}
...
func UpdateCluster(ctx context.Context, db tx, name string, object provisioning.Cluster) (_err error) {
	...
	result, err := stmt.Exec(object.Name, object.ConnectionURL, object.Certificate, object.Status, object.UpdateStatus, object.Channel, object.Description, object.Properties, time.Now().UTC().Format(time.RFC3339), id)
	if err != nil {
