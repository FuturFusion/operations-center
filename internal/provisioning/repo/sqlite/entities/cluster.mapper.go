...
SELECT clusters.id, clusters.name, clusters.connection_url, clusters.certificate, clusters.last_updated
  FROM clusters
...
INSERT INTO clusters (name, connection_url, certificate, last_updated)
  VALUES (?, ?, ?, ?)
`)

var clusterUpdate = RegisterStmt(`
UPDATE clusters
  SET name = ?, connection_url = ?, certificate = ?, last_updated = ?
 WHERE id = ?
`)
...
func clusterColumns() string {
	return "clusters.id, clusters.name, clusters.connection_url, clusters.certificate, clusters.last_updated"
}
...
func getClusters(ctx context.Context, stmt *sql.Stmt, args ...any) ([]provisioning.Cluster, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		...
		err := scan(&c.ID, &c.Name, &c.ConnectionURL, &c.Certificate, &c.LastUpdated)
		if err != nil {
			return err
		}
...
func UpdateCluster(ctx context.Context, db tx, name string, object provisioning.Cluster) (_err error) {
	...
	result, err := stmt.Exec(object.Name, object.ConnectionURL, object.Certificate, object.LastUpdated, id)
	if err != nil {
