...
SELECT servers.id, clusters.name AS cluster, servers.name, servers.type, servers.connection_url, servers.hardware_data, servers.version_data, servers.last_updated
  FROM servers
...
INSERT INTO servers (cluster_id, name, type, connection_url, hardware_data, version_data, last_updated)
  VALUES ((SELECT clusters.id FROM clusters WHERE clusters.name = ?), ?, ?, ?, ?, ?, ?)
`)

var serverUpdate = RegisterStmt(`
UPDATE servers
  SET cluster_id = (SELECT clusters.id FROM clusters WHERE clusters.name = ?), name = ?, type = ?, connection_url = ?, hardware_data = ?, version_data = ?, last_updated = ?
 WHERE id = ?
`)
...
func GetServer(ctx context.Context, db dbtx, name string) (_ *Server, _err error) {
	defer func() {
		_err = mapErr(_err, "Server")
	}()
...
func serverColumns() string {
	return "servers.id, clusters.name AS cluster, servers.name, servers.type, servers.connection_url, servers.hardware_data, servers.version_data, servers.last_updated"
}
...
func getServers(ctx context.Context, stmt *sql.Stmt, args ...any) ([]Server, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		s := Server{}
		var hardwareDataStr string
		err := scan(&s.ID, &s.Cluster, &s.Name, &s.Type, &s.ConnectionURL, &hardwareDataStr, &s.VersionData, &s.LastUpdated)
		if err != nil {
			return err
		}

		err = unmarshal(hardwareDataStr, &s.HardwareData)
		if err != nil {
			return err
		}
...
func getServersRaw(ctx context.Context, db dbtx, sql string, args ...any) ([]Server, error) {
	objects := make([]Server, 0)
...
func GetServers(ctx context.Context, db dbtx, filters ...ServerFilter) (_ []Server, _err error) {
	defer func() {
		_err = mapErr(_err, "Server")
	}()
...
func CreateServer(ctx context.Context, db dbtx, object Server) (_ int64, _err error) {
	defer func() {
		_err = mapErr(_err, "Server")
	}()
	...
	marshaledHardwareData, err := marshal(object.HardwareData)
	if err != nil {
		return -1, err
	}

	args[4] = marshaledHardwareData
	args[5] = object.VersionData
...
func UpdateServer(ctx context.Context, db dbtx, name string, object Server) (_err error) {
	...
	result, err := stmt.Exec(object.Cluster, object.Name, object.Type, object.ConnectionURL, marshaledHardwareData, object.VersionData, object.LastUpdated, id)
	if err != nil {
