...
SELECT servers.id, clusters.name AS cluster, servers.name, servers.type, servers.connection_url, servers.public_connection_url, servers.certificate, clusters.certificate AS cluster_certificate, clusters.connection_url AS cluster_connection_url, servers.hardware_data, servers.os_data, servers.version_data, channels.name AS channel, servers.status, servers.status_detail, servers.description, servers.properties, servers.bmc_api_type, servers.bmc_endpoint, servers.bmc_username, servers.bmc_password, servers.registration_token, servers.last_updated, servers.last_seen, servers.last_status_updated
  FROM servers
...
INSERT INTO servers (cluster_id, name, type, connection_url, public_connection_url, certificate, hardware_data, os_data, version_data, channel_id, status, status_detail, description, properties, bmc_api_type, bmc_endpoint, bmc_username, bmc_password, registration_token, last_updated, last_seen, last_status_updated)
...
UPDATE servers
  SET cluster_id = (SELECT clusters.id FROM clusters WHERE clusters.name = ?), name = ?, type = ?, connection_url = ?, public_connection_url = ?, certificate = ?, hardware_data = ?, os_data = ?, version_data = ?, channel_id = (SELECT channels.id FROM channels WHERE channels.name = ?), status = ?, status_detail = ?, description = ?, properties = ?, bmc_api_type = ?, bmc_endpoint = ?, bmc_username = ?, bmc_password = ?, registration_token = ?, last_updated = ?, last_seen = ?, last_status_updated = ?
 WHERE id = ?
`)
...
func GetServers(ctx context.Context, db dbtx, filters ...provisioning.ServerFilter) (_ []provisioning.Server, _err error) {
	...
	for i, filter := range filters {
		if filter.Status != nil && filter.StatusDetail != nil && filter.ID == nil && filter.Name == nil && filter.Cluster == nil && filter.Certificate == nil && filter.Type == nil {
		...
		} else if filter.Cluster != nil && filter.Status != nil && filter.ID == nil && filter.Name == nil && filter.StatusDetail == nil && filter.Certificate == nil && filter.Type == nil {
			...
			_, where, _ := strings.Cut(parts[0], "WHERE")
			queryParts[0] += "OR" + where
		} else if filter.Cluster != nil && filter.Name != nil && filter.ID == nil && filter.Status == nil && filter.StatusDetail == nil && filter.Certificate == nil && filter.Type == nil {
			...
			_, where, _ := strings.Cut(parts[0], "WHERE")
			queryParts[0] += "OR" + where
		} else if filter.Type != nil && filter.ID == nil && filter.Name == nil && filter.Cluster == nil && filter.Status == nil && filter.StatusDetail == nil && filter.Certificate == nil {
			...
			_, where, _ := strings.Cut(parts[0], "WHERE")
			queryParts[0] += "OR" + where
		} else if filter.Status != nil && filter.ID == nil && filter.Name == nil && filter.Cluster == nil && filter.StatusDetail == nil && filter.Certificate == nil && filter.Type == nil {
			...
			_, where, _ := strings.Cut(parts[0], "WHERE")
			queryParts[0] += "OR" + where
		} else if filter.Name != nil && filter.ID == nil && filter.Cluster == nil && filter.Status == nil && filter.StatusDetail == nil && filter.Certificate == nil && filter.Type == nil {
			...
			_, where, _ := strings.Cut(parts[0], "WHERE")
			queryParts[0] += "OR" + where
		} else if filter.Cluster != nil && filter.ID == nil && filter.Name == nil && filter.Status == nil && filter.StatusDetail == nil && filter.Certificate == nil && filter.Type == nil {
			...
			_, where, _ := strings.Cut(parts[0], "WHERE")
			queryParts[0] += "OR" + where
		} else if filter.Certificate != nil && filter.ID == nil && filter.Name == nil && filter.Cluster == nil && filter.Status == nil && filter.StatusDetail == nil && filter.Type == nil {
			...
			_, where, _ := strings.Cut(parts[0], "WHERE")
			queryParts[0] += "OR" + where
		} else if filter.ID == nil && filter.Name == nil && filter.Cluster == nil && filter.Status == nil && filter.StatusDetail == nil && filter.Certificate == nil && filter.Type == nil {
...
func GetServerNames(ctx context.Context, db dbtx, filters ...provisioning.ServerFilter) (_ []string, _err error) {
	...
	for i, filter := range filters {
		if filter.Cluster != nil && filter.ID == nil && filter.Name == nil && filter.Status == nil && filter.StatusDetail == nil && filter.Certificate == nil && filter.Type == nil {
			args = append(args, []any{filter.Cluster}...)
			if len(filters) == 1 {
...
func UpdateServer(ctx context.Context, db tx, name string, object provisioning.Server) (_err error) {
	...
	result, err := stmt.Exec(object.Cluster, object.Name, object.Type, object.ConnectionURL, object.PublicConnectionURL, object.Certificate, object.HardwareData, object.OSData, object.VersionData, object.Channel, object.Status, object.StatusDetail, object.Description, object.Properties, marshaledBMCAPIType, object.BMCEndpoint, object.BMCUsername, object.BMCPassword, object.RegistrationToken, time.Now().UTC().Format(time.RFC3339), object.LastSeen, object.LastStatusUpdated, id)
	if err != nil {
