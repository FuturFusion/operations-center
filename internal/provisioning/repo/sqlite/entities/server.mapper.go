...
INSERT INTO servers (cluster_id, name, type, connection_url, public_connection_url, certificate, hardware_data, os_data, version_data, channel_id, status, status_detail, description, properties, bmc_api_type, bmc_endpoint, bmc_username, bmc_password, registration_token, system_uuid, machine_id, bmc_server_details, last_updated, last_seen, last_status_updated)
...
UPDATE servers
  SET cluster_id = (SELECT clusters.id FROM clusters WHERE clusters.name = ?), name = ?, type = ?, connection_url = ?, public_connection_url = ?, certificate = ?, hardware_data = ?, os_data = ?, version_data = ?, channel_id = (SELECT channels.id FROM channels WHERE channels.name = ?), status = ?, status_detail = ?, description = ?, properties = ?, bmc_api_type = ?, bmc_endpoint = ?, bmc_username = ?, bmc_password = ?, registration_token = ?, system_uuid = ?, machine_id = ?, bmc_server_details = ?, last_updated = ?, last_seen = ?, last_status_updated = ?
 WHERE id = ?
`)
...
func CreateServer(ctx context.Context, db dbtx, object provisioning.Server) (_ int64, _err error) {
	...
	marshaledBMCAPIType, err := marshalJSON(object.BMCAPIType)
	if err != nil {
		return -1, err
	}

	args[14] = marshaledBMCAPIType
	args[15] = object.BMCEndpoint
	args[16] = object.BMCUsername
	args[17] = object.BMCPassword
	args[18] = object.RegistrationToken
	args[19] = object.SystemUUID
	args[20] = object.MachineID
	marshaledBMCServerDetails, err := marshalJSON(object.BMCServerDetails)
	if err != nil {
		return -1, err
	}

	args[21] = marshaledBMCServerDetails
	args[22] = time.Now().UTC().Format(time.RFC3339)
	args[23] = object.LastSeen
	args[24] = object.LastStatusUpdated
...
func UpdateServer(ctx context.Context, db tx, name string, object provisioning.Server) (_err error) {
	...
	result, err := stmt.Exec(object.Cluster, object.Name, object.Type, object.ConnectionURL, object.PublicConnectionURL, object.Certificate, object.HardwareData, object.OSData, object.VersionData, object.Channel, object.Status, object.StatusDetail, object.Description, object.Properties, marshaledBMCAPIType, object.BMCEndpoint, object.BMCUsername, object.BMCPassword, object.RegistrationToken, object.SystemUUID, object.MachineID, marshaledBMCServerDetails, time.Now().UTC().Format(time.RFC3339), object.LastSeen, object.LastStatusUpdated, id)
	if err != nil {
