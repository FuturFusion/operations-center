...
INSERT INTO servers (cluster_id, name, type, connection_url, public_connection_url, certificate, hardware_data, os_data, version_data, channel_id, status, status_detail, description, properties, bmc_api_type, bmc_endpoint, bmc_username, bmc_password, registration_token, system_uuid, machine_id, last_updated, last_seen, last_status_updated)
...
UPDATE servers
  SET cluster_id = (SELECT clusters.id FROM clusters WHERE clusters.name = ?), name = ?, type = ?, connection_url = ?, public_connection_url = ?, certificate = ?, hardware_data = ?, os_data = ?, version_data = ?, channel_id = (SELECT channels.id FROM channels WHERE channels.name = ?), status = ?, status_detail = ?, description = ?, properties = ?, bmc_api_type = ?, bmc_endpoint = ?, bmc_username = ?, bmc_password = ?, registration_token = ?, system_uuid = ?, machine_id = ?, last_updated = ?, last_seen = ?, last_status_updated = ?
 WHERE id = ?
`)
...
func getServers(ctx context.Context, stmt *sql.Stmt, args ...any) ([]provisioning.Server, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		...
		err := scan(&s.ID, &s.Cluster, &s.Name, &s.Type, &s.ConnectionURL, &s.PublicConnectionURL, &s.Certificate, &s.ClusterCertificate, &s.ClusterConnectionURL, &s.HardwareData, &s.OSData, &s.VersionData, &s.Channel, &s.Status, &s.StatusDetail, &s.Description, &s.Properties, &bMCAPITypeStr, &s.BMCEndpoint, &s.BMCUsername, &s.BMCPassword, &s.RegistrationToken, &s.SystemUUID, &s.MachineID, &s.LastUpdated, &s.LastSeen, &s.LastStatusUpdated)
		if err != nil {
			return err
		}
...
func CreateServer(ctx context.Context, db dbtx, object provisioning.Server) (_ int64, _err error) {
	...
	args[21] = time.Now().UTC().Format(time.RFC3339)
	args[22] = object.LastSeen
	args[23] = object.LastStatusUpdated
...
func UpdateServer(ctx context.Context, db tx, name string, object provisioning.Server) (_err error) {
	...
	result, err := stmt.Exec(object.Cluster, object.Name, object.Type, object.ConnectionURL, object.PublicConnectionURL, object.Certificate, object.HardwareData, object.OSData, object.VersionData, object.Channel, object.Status, object.StatusDetail, object.Description, object.Properties, marshaledBMCAPIType, object.BMCEndpoint, object.BMCUsername, object.BMCPassword, object.RegistrationToken, object.SystemUUID, object.MachineID, time.Now().UTC().Format(time.RFC3339), object.LastSeen, object.LastStatusUpdated, id)
	if err != nil {
