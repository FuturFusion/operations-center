...
UPDATE clusters
  SET name = ?, connection_url = ?, server_hostnames = ?, last_updated = ?
 WHERE id = ?
`)
...
func GetCluster(ctx context.Context, db dbtx, name string) (_ *Cluster, _err error) {
	defer func() {
		_err = mapErr(_err, "Cluster")
	}()
...
func getClusters(ctx context.Context, stmt *sql.Stmt, args ...any) ([]Cluster, error) {
	...
	dest := func(scan func(dest ...any) error) error {
		...
		err = unmarshal(serverHostnamesStr, &c.ServerHostnames)
		if err != nil {
			return err
		}
...
func getClustersRaw(ctx context.Context, db dbtx, sql string, args ...any) ([]Cluster, error) {
	objects := make([]Cluster, 0)
...
func GetClusters(ctx context.Context, db dbtx, filters ...ClusterFilter) (_ []Cluster, _err error) {
	defer func() {
		_err = mapErr(_err, "Cluster")
	}()
...
func CreateCluster(ctx context.Context, db dbtx, object Cluster) (_ int64, _err error) {
	defer func() {
		_err = mapErr(_err, "Cluster")
	}()
	...
	marshaledServerHostnames, err := marshal(object.ServerHostnames)
	if err != nil {
		return -1, err
	}
...
func UpdateCluster(ctx context.Context, db dbtx, name string, object Cluster) (_err error) {
	...
	result, err := stmt.Exec(object.Name, object.ConnectionURL, marshaledServerHostnames, object.LastUpdated, id)
	if err != nil {
