...
func (_d ClusterClientPortWithSlog) EnableCluster(ctx context.Context, server provisioning.Server, clusterName string) (clusterCertificate string, err error) {
	log := _d._log.With()
	...
	return _d._base.EnableCluster(ctx, server, clusterName)
}
...
func (_d ClusterClientPortWithSlog) JoinCluster(ctx context.Context, server provisioning.Server, joinToken string, clusterCertificate string) (err error) {
	log := _d._log.With()
	if _d._log.Enabled(ctx, logger.LevelTrace) {
		log = log.With(
			slog.Any("ctx", ctx),
	...
	return _d._base.JoinCluster(ctx, server, joinToken, clusterCertificate)
}
