...
func (_d BMCServerClientPortWithPrometheus) Dump(ctx context.Context, server provisioning.Server, trace bool) (bMCDump api.BMCDump, err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
	...
	return _d.base.Dump(ctx, server, trace)
}
