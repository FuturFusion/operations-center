...
func (_d ServerClientPortWithSlog) GetServerType(ctx context.Context, endpoint provisioning.Endpoint) (serverType api.ServerType, err error) {
	...
	log.Debug("=> calling GetServerType")
	defer func() {
		log := _d._log.With()
		if _d._log.Enabled(ctx, logger.LevelTrace) {
			log = _d._log.With(
		...
		if err != nil {
			if _d._isInformativeErrFunc(err) {
				log.Debug("<= method GetServerType returned an informative error")
			} else {
				log.Error("<= method GetServerType returned an error")
			}
		} else {
			log.Debug("<= method GetServerType finished")
		}
	}()
