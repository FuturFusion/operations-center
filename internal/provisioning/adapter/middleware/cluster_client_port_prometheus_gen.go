...
func (_d ClusterClientPortWithPrometheus) EnableCluster(ctx context.Context, server provisioning.Server, clusterName string) (clusterCertificate string, err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
	...
	return _d.base.EnableCluster(ctx, server, clusterName)
}
...
func (_d ClusterClientPortWithPrometheus) JoinCluster(ctx context.Context, server provisioning.Server, joinToken string, clusterCertificate string) (err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
	...
	return _d.base.JoinCluster(ctx, server, joinToken, clusterCertificate)
}
