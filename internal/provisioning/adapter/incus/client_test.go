...
func TestClient_InsecureGetClusterCertificate(t *testing.T) {
	certPEMByte, keyPEMByte, err := incustls.GenerateMemCert(true, false)
	require.NoError(t, err)
	...
	tests := []struct {
		...
		assertErr      require.ErrorAssertionFunc
		wantPath       string
		wantServerCert func(serverCert string) (expectedServerCert string)
	}{
		{
			name:       "success",
			certPEM:    certPEM,
  ...
  "metadata": {}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			target := provisioning.Server{
				ConnectionURL: server.URL,
				// Certificate:   string(serverCert), // Don't provide the certificate, since it is expected to work without.
			}

			// Run test
			gotClusterCertificate, err := client.InsecureGetClusterCertificate(ctx, target)

			// Assert
			tc.assertErr(t, err)
			require.Equal(t, tc.wantPath, gotPath)
			require.Equal(t, tc.wantServerCert(string(serverCert)), gotClusterCertificate)
		})
	}
}
