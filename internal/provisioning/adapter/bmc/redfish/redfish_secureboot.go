...
// database, from what the BIOS profiles of a server resolved to. The profiles
// are the only source, so an entry with a value of false is not allow listed,
// just like one, that no profile mentions at all.
