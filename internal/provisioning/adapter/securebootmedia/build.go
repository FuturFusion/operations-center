...
func MediaID(imageType api.ImageType, architecture images.UpdateFileArchitecture, certificates provisioning.SecureBootCertificates) string {
	return provisioning.SeedImageFingerprintID(mediaFingerprint(imageType, architecture, certificates))
}

func mediaFingerprint(imageType api.ImageType, architecture images.UpdateFileArchitecture, certificates provisioning.SecureBootCertificates) string {
	sum := sha256.New()

	parts := make([]string, 0, 3+2*len(secureBootDatabases))
	parts = append(parts, string(imageType), string(architecture))
...
// buildImage wraps the EFI system partition in a partition table and returns the
// path of the enrollment media.
//
// The table is laid out for the logical block size the media is read in, which
// depends on the kind of media the BMC emulates for it: 2048 byte blocks for a
// CD, 512 byte ones for a USB stick. Firmware, that only finds a table laid out
// for the block size of the device it reads, finds nothing at all otherwise.
func (m *Media) buildImage(ctx context.Context, dir string, imageType api.ImageType, id string, espFile string) (string, error) {
	blockSize, err := blockSizeForImageType(imageType)
	if err != nil {
		return "", err
	}
	...
	imageFile := filepath.Join(dir, "media"+imageType.FileExt())
	...
	err = m.runTool(ctx, "systemd-repart",
		...
		"--empty=create",
		"--size=auto",
		"--sector-size="+strconv.Itoa(blockSize),
...
// mediaBlockSizes are the logical block sizes the generated media is laid out
// for, by the kind of media a BMC emulates for an image type: an ISO is
// attached as a CD, which is read in 2048 byte blocks, a raw image as a USB
// stick, which is read in the 512 byte blocks of a disk.
var mediaBlockSizes = map[api.ImageType]int{
	api.ImageTypeISO: 2048,
	api.ImageTypeRaw: 512,
}

// blockSizeForImageType returns the logical block size the media of an image
// type is laid out for.
func blockSizeForImageType(imageType api.ImageType) (int, error) {
	blockSize, ok := mediaBlockSizes[imageType]
	if !ok {
		return 0, domain.NewErrorf(domain.ErrOperationNotPermitted, "", "Generating the secure boot enrollment media is not supported for the image type %q", imageType).
