...
func (s clusterService) StartLifecycleEventsMonitor(ctx context.Context) error {
	...
	s.clusterUpdateSignal.AddListener(func(ctx context.Context, cum ClusterUpdateMessage) {
		...
		// TODO: handle cluster updates, in particular, if the connection URL changes.
			...
			for {
				...
				case event := <-events:
					for _, inventorySyncer := range s.inventorySyncers {
						...
						if err != nil {
							slog.WarnContext(ctx, "Failed to resync", slog.Any("resource_type", event.ResourceType), slog.String("source", event.Source.String()), logger.Err(err))
						}
					}
