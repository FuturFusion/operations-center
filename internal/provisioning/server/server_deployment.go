...
func (s *serverService) DeployByName(ctx context.Context, name string, request provisioning.ServerDeploymentRequest) error {
	...
	err = transaction.Do(ctx, func(ctx context.Context) error {
		...
		if request.SecureBootEnrollmentMedia {
			err = s.checkSecureBootEnrollmentMediaSupported(ctx, *server, request.Architecture)
			if err != nil {
				return err
			}
		}
...
func (s *serverService) checkSecureBootEnrollmentMediaSupported(ctx context.Context, server provisioning.Server, architecture images.UpdateFileArchitecture) error {
	...
	return s.secureBootMedia.CheckSupported(ctx, architecture)
}
