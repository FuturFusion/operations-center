...
// contributes its skip reason, the state, that is finally entered, its enter
// reason.
func deploymentDiagramTarget(deployment *provisioning.ServerDeployment, next api.ServerDeploymentState) (api.ServerDeploymentState, []string) {
	...
	for range len(deploymentStates) {
		...
		if definition.skipReason != "" {
			reasons = append(reasons, definition.skipReason)
		}
...
// deploymentDiagramOrder lays the states out along the happy path, so the
// rendered diagram does not churn with the iteration order of the table.
func deploymentDiagramOrder() []api.ServerDeploymentState {
	var order []api.ServerDeploymentState
...
// request are deliberately left out: every state has both, and drawing 37 of
// each would bury the machine. The prose next to the diagram says so.
