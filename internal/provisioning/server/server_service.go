...
func WithSecureBootMediaPort(media provisioning.SecureBootMediaPort, source provisioning.SecureBootCertificateSourcePort, catalog provisioning.SecureBootCertificateCatalogPort) Option {
	return func(s *serverService) {
		s.secureBootMedia = media
		s.secureBootCertificate = source
		s.secureBootCatalog = catalog
	}
}
...
func (s *serverService) secureBootEnrollmentCertificates(ctx context.Context, log *slog.Logger, secureBoot api.BIOSSecureBoot) (provisioning.SecureBootCertificates, error) {
	...
	// The platform key is enrolled from the update IncusOS ships rather than
	// from its certificate, since firmware, that requires the platform key to be
	// self signed, only accepts the update signed by the platform key itself.
	platformKeyUpdate, err := s.secureBootCertificate.GetSecureBootPlatformKeyUpdate(ctx)
	if err != nil {
		return provisioning.SecureBootCertificates{}, fmt.Errorf("Failed to get the secure boot platform key update from IncusOS: %w", err)
	}
...
func (s *serverService) bmcAttachSecureBootMediaByName(ctx context.Context, log *slog.Logger, server provisioning.Server, imageType api.ImageType, architecture images.UpdateFileArchitecture, secureBoot api.BIOSSecureBoot, virtualMediaID string) (bmcAttachedMedia, error) {
	...
	if base == "" {
		return bmcAttachedMedia{}, domain.NewErrorf(domain.ErrOperationNotPermitted, "", "Operations Center address is not configured, cannot build the secure boot enrollment media URL").
