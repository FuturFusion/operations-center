...
func NewServerService(repo ServerRepo, client ServerClientPort, tokenSvc TokenService, clusterSvc ClusterService, updateSvc UpdateService, serverConnectionURL string, serverCertificate tls.Certificate, opts ...ServerServiceOption) *serverService {
...
func (s *serverService) UpdateSystemUpdate(ctx context.Context, name string, updateConfig ServerSystemUpdate) error {
	_, err := s.updateSvc.GetExposedchannelByName(ctx, updateConfig.Config.Channel)
	if err != nil {
		return fmt.Errorf("Failed to get exposed channel %q: %w", name, err)
	}
