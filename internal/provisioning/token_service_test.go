...
func TestTokenService_GetPreSeedImage(t *testing.T) {
	...
	tests := []struct {
		...
		seedConfigArg                         provisioning.TokenImageSeeds
		repoGetByUUIDErr                      error
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			flasherAdapter := &adapterMock.FlasherPortMock{
				GenerateSeededImageFunc: func(ctx context.Context, id uuid.UUID, seedConfig provisioning.TokenImageSeeds, rc io.ReadCloser) (io.ReadCloser, error) {
