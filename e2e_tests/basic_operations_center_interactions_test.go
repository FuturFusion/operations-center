...
func ocIncusImagesRemoteLaunchInstance(names []string) func(t *testing.T, tmpDir string) {
	return func(t *testing.T, tmpDir string) {
		...
		mustRun(t, `../bin/operations-center.linux.%[1]s image incus add alpine:edge:%[1]s:default %[2]s %[3]s/incus.tar.xz %[3]s/root.squashfs %[3]s/disk.qcow2`, cpuArch, currentAlpineVersion, tmpImagesDir)
